...
function getFromLogin({html, cridentials}) {
  ...
  post.password = hex_sha1(hex_md5(password));
  post.language = 'fr_FR';
  return post;
}
...
function checkCookie(html) {
  const $ = cheerio.load(html);
  const isMenus = $('#masterdiv > div:nth-child(1) > a').text();
  if (isMenus) {
    return true;
  }
  return false;
...
function checkLogin(html) {
  let error = html.match(/errorMessage\=\"+.+\"\;/g);
  if (error) {
    error = error[0].replace('errorMessage="', '').replace('";', '').trim();
    if (/Invalid/ig.test(error)) {
      throw new Error('User atau Password salah.')
    }else if (/User is still login/ig.test(error)) {
      throw new Error('User telah login')
    }else {
      throw new Error(`Error 100 ${error}`)
    }
  }
  return true;
}
